...
class Rule(object):
    def __init__(self, tokens, line_num, comments):
        ...
        if self.wl != None:
            ids = list(set(map(int, self.wl[3:].split(","))))
            ids.sort()
            ids = map(str, ids)
            self.wl = "wl:" + ",".join(ids)
        else:
    ...
    def validate(self):
        if self.main_rule == None and self.basic_rule == None:
            print(
                "ERROR: {}: missing 'MainRule' or 'BasicRule' prefix".format(
                    self.line_num
                )
            )
            sys.exit(1)
...
def parse_file(filename, rules, whitelists, ruleid):
    ...
    for line in lines:
        ...
        if not line.endswith(";"):
            print(
                "ERROR: {}: missing ; at the end the line '{}'".format(line_num, line)
            )
            sys.exit(1)
        ...
        if ruleid > 100:
            ...
            print(
                "ERROR: {}: Rule {} already exists at line {}".format(
                    line_num, rule.id, rules[rule.id].line_num
                )
            )
            sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        ...
        "--format",
        default="rules",
        help="format of the output file ({})".format(
            ",".join(list(output_formats.keys()))
        ),
    )
    parser.add_argument(
        "-b",
        ...
        default=0,
        type=int,
        help="rebase all rules ids from this id number (id must be > 100)",
    )
    ...
    if (
        ...
        or len(args.format) < 1
        or args.format not in output_formats
        or (args.begin_id != 0 and args.begin_id <= 100)
    ):
