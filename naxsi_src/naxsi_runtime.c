                             ...
                             ngx_int_t               target_name)
{
  ...
  if (ctx->extensive_log) {
    if (target_name) {
      naxsi_log_offending_extra(ctx, req, value, name, r, zone, target_name);
    } else {
      naxsi_log_offending_extra(ctx, req, name, value, r, zone, target_name);
    }
  }
  if (nb_match == 0)
    nb_match = 1;
