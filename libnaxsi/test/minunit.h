...
void snprint_mem(char *out, size_t out_size, const unsigned char *buf, size_t len) {
	size_t i;
	*out = '\0';
	for (i = 0; i < len; i++) {
		size_t out_len;
...
#define mu_assert_eq_fmt(actual, expected, message, fmt) \
	do { \
		if ((actual) != (expected)) { \
			char _meqstr[MU_BUF_SIZE]; \
			snprintf(_meqstr, MU_BUF_SIZE, "%s: expected " fmt ", got " fmt ".", (message), (actual), (expected)); \
			mu_assert(_meqstr, false); \
		} \
	} while (0)
...
#define mu_assert_memeq(actual, expected, len, message) \
	do { \
		char _meqstr[MU_BUF_SIZE]; \
		size_t _meqstr_len; \
		const unsigned char *act__ = (actual); \
		const unsigned char *exp__ = (expected); \
